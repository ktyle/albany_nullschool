...
var products = function() {
	...
	console.log ("attr.level= " + attr.level);
...
console.log ("attr.surface = " + attr.surface);
    ...
    var FACTORIES = {
        ...
        "tropIrr": {
            matches: _.matches({param: "tropIrr"}),
            create: function(attr) {
                return buildProduct({
                    field: "vector",
                    type: "tropIrr",
                    description: localize({
                        name: {en: "Irr_Wind", ja: "風速"},
                        qualifier: {en: " @ " + describeSurface(attr), ja: " @ " + describeSurfaceJa(attr)}
                    }),
                    paths: [gfs1p0degPath(attr, "tropIrr", attr.surface, attr.level)],
                    date: gfsDate(attr),
                    builder: function(file) {
                        var uData = file[0].data, vData = file[1].data;
                        return {
        ...
        "twspd": {
            matches: _.matches({param: "wind", overlayType: "twspd"}),
            create: function(attr) {
                return buildProduct({
                    field: "scalar",
                    type: "twspd",
                    description: localize({
                        name: {en: "Wspd on Trop", ja: "気温"},
                        qualifier: {en: " @ " + describeSurface(attr), ja: " @ " + describeSurfaceJa(attr)}
                    }),
                    paths: [gfs1p0degPath(attr, "twspd", attr.surface, attr.level)],
                    date: gfsDate(attr),
                    builder: function(file) {
                        var record = file[0], data = record.data;
                        return {
        ...
        "theta": {
            matches: _.matches({param: "wind", overlayType: "theta"}),
            create: function(attr) {
                return buildProduct({
                    field: "scalar",
                    type: "theta",
                    ...
                    paths: [gfs1p0degPath(attr, "theta", attr.surface, attr.level)],
                    date: gfsDate(attr),
                    builder: function(file) {
                        var record = file[0], data = record.data;
                        return {
